...
switch(method)
    case 'CFP-rand'
        model = CFP_rand(d,bit,2,1);
        tic;
        B1 = CFP_prediction(model, Xtraining);
        B2 = CFP_prediction(model, Xtest);
        toc;
    ...
    case 'LSH-rand-orth'
        ...
        % ITQ
        [~, R] = ITQ_rand(Xtraining(1:train_size,:));
        B1 = Xtraining*R;
        B2 = Xtest*R;
        B1 = compactbit(B1>0);
        B2 = compactbit(B2>0);
    ...
    case 'BBE-opt' % BITQLOW
        ...
        XX_train = TensorFV(Xtraining);
        XX_test = TensorFV(Xtest);
        [R1,R2] = BilinearITQ_low(XX_train(:,:,1:train_size),b1,b2,para.iter);
   ...
   case 'KBE-opt-2'
        model = Kronecker_rand(d,bit,2,1);
        tic;
        [B1,model] = KBE_optimized(model, Xtraining, para.iter);
        B2 = KBE_prediction(model, Xtest);
        toc;
    ...
    case 'KBE-opt-4'
        model = Kronecker_rand(d,bit,4,1);
        tic;
        [B1,model] = KBE_optimized(model, Xtraining, para.iter);
        B2 = KBE_prediction(model, Xtest);
        toc;
    ...
    case 'KBE-rand-4'      
        model = Kronecker_rand(d,bit,4,0);
        tic;
        B1 = KBE_prediction(model, Xtraining);
        B2 = KBE_prediction(model, Xtest);
        toc;
